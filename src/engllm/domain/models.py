"""Typed domain models for the EngLLM pipeline."""
...
class WorkspaceConfig(DomainModel):
    """Shared artifact-root defaults for all EngLLM tools."""
...
class SDDToolDefaults(DomainModel):
    """Default configuration for the SDD tool."""

    template: Path


class AskToolDefaults(DomainModel):
    """Reserved defaults block for the ask tool namespace."""

    interactive_top_k: int = 6

    @field_validator("interactive_top_k")
    @classmethod
    def validate_interactive_top_k(cls, value: int) -> int:
        """Reject zero/negative interactive top-k defaults."""

        if value <= 0:
            raise ValueError("interactive_top_k must be a positive integer")
        return value


class ToolDefaults(DomainModel):
    """Per-tool defaults kept under one shared toolkit config."""

    sdd: SDDToolDefaults
    ask: AskToolDefaults = Field(default_factory=AskToolDefaults)


class ProjectConfig(DomainModel):
    """Top-level EngLLM toolkit configuration."""
