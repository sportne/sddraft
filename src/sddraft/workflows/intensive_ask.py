...
from sddraft.analysis.retrieval import resolve_retrieval_store_path, to_citations
from sddraft.domain.errors import ConfigError, RenderingError
...
def answer_question_intensive(
    *,
    ...
    if project_config is None:
        raise ConfigError(
            "Intensive ask mode requires --project-config so repo scope can be resolved."
        )

    retrieval_root = resolve_retrieval_store_path(index_path)
    output_root = retrieval_root.parent
    resolved_repo_root = (repo_root or Path(".")).resolve()
    csc_id = output_root.name
    question_hash = _question_hash(request)
    run_root = default_intensive_runs_root(output_root) / question_hash
    ...
    corpus_manifest_path = default_intensive_corpus_root(output_root) / "manifest.json"
    ...
    write_json_model(output_root / "run_metrics_ask.json", metrics_collector.metrics)
