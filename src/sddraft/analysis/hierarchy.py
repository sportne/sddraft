"""Deterministic hierarchy index helpers for hierarchy docs and ask expansion."""
...
def default_hierarchy_index_path(index_path: Path) -> Path:
    """Return conventional hierarchy index path for retrieval paths."""

    if index_path.is_dir() and index_path.name == "retrieval":
        return index_path.parent / "hierarchy" / "hierarchy_index.json"
    if index_path.is_file() and index_path.name == "manifest.json":
        if index_path.parent.name == "retrieval":
            return index_path.parent.parent / "hierarchy" / "hierarchy_index.json"
        return index_path.parent / "hierarchy" / "hierarchy_index.json"
    if index_path.is_file() and index_path.name == "retrieval_index.json":
        return index_path.parent / "hierarchy" / "hierarchy_index.json"
    if (index_path / "retrieval").exists():
        return index_path / "hierarchy" / "hierarchy_index.json"
    return index_path.parent / "hierarchy" / "hierarchy_index.json"
...
def load_hierarchy_index(path: Path) -> HierarchyIndex:
    """Load hierarchy index from JSON file."""

    if not path.exists():
        raise AnalysisError(f"Hierarchy index not found: {path}")
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AnalysisError(f"Invalid hierarchy index JSON at {path}: {exc}") from exc
    return HierarchyIndex.model_validate(raw)
