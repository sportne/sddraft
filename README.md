...
```bash
sddraft ask --index-path artifacts/NAV_CTRL/retrieval --question "What interfaces are exposed?" --provider mock
```
...
sddraft ask --index-path artifacts/NAV_CTRL/retrieval --question "What interfaces are exposed?" --provider ollama --model qwen2.5:14b-instruct-q4_K_M
```
...
```bash
sddraft migrate-index --index-path artifacts/NAV_CTRL/retrieval_index.json
```

After migration, use `--index-path artifacts/NAV_CTRL/retrieval` with `sddraft ask`.
