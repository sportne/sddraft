...
- [x] **H11-03**  
  ...
  `Definition of Done:` The tool writes `semantic_context_map.json` during normal builds, promotes public `engllm history-docs build` rendering of `System Context` / `Interfaces` through the combined H11-03 path, keeps plain H11-02 semantic clustering shadow-only, and supports H10 real-repo promotion-gate evaluation against the path baseline.  
...
- [x] **H12-01**  
  ...
  `Definition of Done:` The tool emits checkpoint-scoped `interval_interpretation.json` artifacts with structured design-change insights, rationale clues, and significant change windows on top of deterministic H3 evidence, while leaving H4-H9 behavior unchanged by default.
